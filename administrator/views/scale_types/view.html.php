 ...
 */
class HrmViewScale_types extends JViewLegacy {
    ...
    public function display($tpl = null) {
        ...
        HrmHelper::addSubmenu('scale_types');
    ...
    protected function addToolbar() {
        ...
        JToolBarHelper::title(JText::_('COM_HRM_TITLE_SCALE_TYPES'), 'scale_types.png');

        //Check if the form exists before showing the add/edit buttons
        $formPath = JPATH_COMPONENT_ADMINISTRATOR . '/views/scale_type';
        if (file_exists($formPath)) {

            if ($canDo->get('core.create')) {
                JToolBarHelper::addNew('scale_type.add', 'JTOOLBAR_NEW');
            }

            if ($canDo->get('core.edit') && isset($this->items[0])) {
                JToolBarHelper::editList('scale_type.edit', 'JTOOLBAR_EDIT');
            }
        }
        ...
        if ($canDo->get('core.edit.state')) {
            ...
            if (isset($this->items[0]->state)) {
                ...
                JToolBarHelper::custom('scale_types.publish', 'publish.png', 'publish_f2.png', 'JTOOLBAR_PUBLISH', true);
                JToolBarHelper::custom('scale_types.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JTOOLBAR_UNPUBLISH', true);
            } else if (isset($this->items[0])) {
                //If this component does not use state then show a direct delete button as we can not trash
                JToolBarHelper::deleteList('', 'scale_types.delete', 'JTOOLBAR_DELETE');
            }

            if (isset($this->items[0]->state)) {
                JToolBarHelper::divider();
                JToolBarHelper::archiveList('scale_types.archive', 'JTOOLBAR_ARCHIVE');
            }
            if (isset($this->items[0]->checked_out)) {
                JToolBarHelper::custom('scale_types.checkin', 'checkin.png', 'checkin_f2.png', 'JTOOLBAR_CHECKIN', true);
            }
        }
        ...
        if (isset($this->items[0]->state)) {
            if ($state->get('filter.state') == -2 && $canDo->get('core.delete')) {
                JToolBarHelper::deleteList('', 'scale_types.delete', 'JTOOLBAR_EMPTY_TRASH');
                JToolBarHelper::divider();
            } else if ($canDo->get('core.edit.state')) {
                JToolBarHelper::trash('scale_types.trash', 'JTOOLBAR_TRASH');
                JToolBarHelper::divider();
            }
        }
        ...
        JHtmlSidebar::setAction('index.php?option=com_hrm&view=scale_types');
        ...
        $form = JForm::getInstance('com_hrm.scale_type', 'scale_type');
	...
	protected function getSortFields()
	{
		...
		'a.title' => JText::_('COM_HRM_SCALE_TYPES_TITLE'),
		'a.code' => JText::_('COM_HRM_SCALE_TYPES_CODE'),
		'a.scale_group_guid' => JText::_('COM_HRM_SCALE_TYPES_SCALE_GROUP_GUID'),
		);
	}
