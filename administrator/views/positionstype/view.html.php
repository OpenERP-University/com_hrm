 ...
 * View to edit
 */
class HrmViewPositionstype extends JViewLegacy {
    ...
    protected function addToolbar() {
        ...
        JToolBarHelper::title(JText::_('COM_HRM_TITLE_POSITIONSTYPE'), 'positionstype.png');

        // If not checked out, can save the item.
        if (!$checkedOut && ($canDo->get('core.edit') || ($canDo->get('core.create')))) {

            JToolBarHelper::apply('positionstype.apply', 'JTOOLBAR_APPLY');
            JToolBarHelper::save('positionstype.save', 'JTOOLBAR_SAVE');
        }
        if (!$checkedOut && ($canDo->get('core.create'))) {
            JToolBarHelper::custom('positionstype.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
        }
        // If an existing item, can save to a copy.
        if (!$isNew && $canDo->get('core.create')) {
            JToolBarHelper::custom('positionstype.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
        }
        if (empty($this->item->id)) {
            JToolBarHelper::cancel('positionstype.cancel', 'JTOOLBAR_CANCEL');
        } else {
            if ($this->state->params->get('save_history', 0) && $user->authorise('core.edit')) {
                JToolbarHelper::versions('com_hrm.positionstype', $this->item->id);
            }
            JToolBarHelper::cancel('positionstype.cancel', 'JTOOLBAR_CLOSE');
        }
    }
