 ...
 */
class HrmModelRewarddisciplines extends JModelList {
    ...
    protected function populateState($ordering = null, $direction = null) {
		...
		//Filtering option
		$this->setState('filter.option', $app->getUserStateFromRequest($this->context.'.filter.option', 'filter_option', '', 'string'));
    ...
    protected function getListQuery() {
        ...
        $query->from('`#__hrm_reward_discipline` AS a');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.employee_guid LIKE '.$search.'  OR  a.department_guid LIKE '.$search.'  OR  a.option LIKE '.$search.'  OR  a.note LIKE '.$search.'  OR  a.date LIKE '.$search.' )');
            }
        }
		...
		//Filtering option
		$filter_option = $this->state->get("filter.option");
		if ($filter_option) {
			$query->where("a.option = '".$db->escape($filter_option)."'");
		}
    ...
    public function getItems() {
		...
		foreach ($items as $oneItem) {
			...
			$oneItem->department_guid = !empty($textValue) ? implode(', ', $textValue) : $oneItem->department_guid;
					...
					$oneItem->option = JText::_('COM_HRM_REWARDDISCIPLINES_OPTION_OPTION_' . strtoupper($oneItem->option));
		}
        return $items;
    }
