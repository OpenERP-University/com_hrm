 ...
 * Hrm model.
 */
class HrmModelWage extends JModelAdmin
{
	/**
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_hrm.wage', 'wage', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__hrm_wage');
				$max = $db->loadResult();
	 ...
	 public function getListWare($ware_max = NULL) {
        if ($ware_max) {
            $db = JFactory::getDbo();
            ...
            $query
                    ...
                    ->from('`#__hrm_wage`')
                    ->where($db->quoteName('title') . ' <= ' . $db->quote($db->escape($ware_max)));
