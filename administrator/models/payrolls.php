...
class HrmModelPayrolls extends JModelList {
    ...
    protected function populateState($ordering = null, $direction = null) {
		...
		$this->setState('filter.scale_type_guid', $app->getUserStateFromRequest($this->context.'.filter.scale_type_guid', 'filter_scale_type_guid', '', 'string'));
    ...
    protected function getListQuery() {
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.employee_guid LIKE '.$search.'  OR  a.scale_group_guid LIKE '.$search.'  OR  a.scale_type_guid LIKE '.$search.'  OR  a.wage_guid LIKE '.$search.' )');
            }
        }
    ...
    public function getItems() {
		...
		foreach ($items as $oneItem) {
			...
			if (isset($oneItem->scale_type_guid)) {
				$values = explode(',', $oneItem->scale_type_guid);
			...
			$oneItem->scale_type_guid = !empty($textValue) ? implode(', ', $textValue) : $oneItem->scale_type_guid;
			...
			$oneItem->wage_guid = !empty($textValue) ? implode(', ', $textValue) : $oneItem->wage_guid;
					...
					$oneItem->employee_type = JText::_('COM_HRM_PAYROLLS_EMPLOYEE_TYPE_OPTION_' . strtoupper($oneItem->employee_type));
		}
        return $items;
    }
