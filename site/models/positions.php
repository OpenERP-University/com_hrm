 ...
 */
class HrmModelPositions extends JModelList
{
...
if (empty($list['direction']))
{
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__hrm_position` AS a');
    ...
    $query->join('LEFT', '#__users AS uc ON uc.id=a.checked_out');
		...
		$query->select('#__hrm_positionstype_12345.title AS positionstypes_title_12345');
		$query->join('LEFT', '#__hrm_positionstype AS #__hrm_positionstype_12345 ON #__hrm_positionstype_12345.guid = a.positiontype_guid');
...
if (!JFactory::getUser()->authorise('core.edit.state', 'com_hrm'))
{
	$query->where('a.state = 1');
		...
		//Filtering positiontype_guid
		$filter_positiontype_guid = $this->state->get("filter.positiontype_guid");
		if ($filter_positiontype_guid) {
			$query->where("a.positiontype_guid = '".$db->escape($filter_positiontype_guid)."'");
		}
	...
	public function getItems()
	{
		...
		foreach($items as $item){
			...
			if (isset($item->positiontype_guid) && $item->positiontype_guid != '') {
				if(is_object($item->positiontype_guid)){
					$item->positiontype_guid = JArrayHelper::fromObject($item->positiontype_guid);
				}
				$values = (is_array($item->positiontype_guid)) ? $item->positiontype_guid : explode(',',$item->positiontype_guid);
			...
			$item->positiontype_guid = !empty($textValue) ? implode(', ', $textValue) : $item->positiontype_guid;
