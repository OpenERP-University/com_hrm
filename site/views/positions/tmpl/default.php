...
<form action="<?php echo JRoute::_('index.php?option=com_hrm&view=positions'); ?>" method="post" name="adminForm" id="adminForm">
    ...
    <?php endif; ?>
    				...
    				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_HRM_POSITIONS_EMPLOYEE_GUID', 'a.employee_guid', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_HRM_POSITIONS_POSITIONTYPE_GUID', 'a.positiontype_guid', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_HRM_POSITIONS_DEPARTMENT_GUID', 'a.department_guid', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_HRM_POSITIONS_START', 'a.start', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_HRM_POSITIONS_END', 'a.end', $listDirn, $listOrder); ?>
				</th>
    ...
    <?php endif; ?>
					...
					<th class="center">
				<?php echo JText::_('COM_HRM_POSITIONS_ACTIONS'); ?>
				</th>
				<?php endif; ?>
    ...
    <?php foreach ($this->items as $i => $item) : ?>
        ...
        <tr class="row<?php echo $i % 2; ?>">
            ...
            <?php if (isset($this->items[0]->state)): ?>
                ...
                <td class="center">
                    <a class="btn btn-micro <?php echo $class; ?>"
                       href="<?php echo ($canEdit || $canChange) ? JRoute::_('index.php?option=com_hrm&task=position.publish&id=' . $item->id . '&state=' . (($item->state + 1) % 2), false, 2) : '#'; ?>">
                        <?php if ($item->state == 1): ?>
            ...
            <?php endif; ?>
					...
					<td class="center">
						<?php if ($canEdit): ?>
							<a href="<?php echo JRoute::_('index.php?option=com_hrm&task=positionform.edit&id=' . $item->id, false, 2); ?>" class="btn btn-mini" type="button"><i class="icon-edit" ></i></a>
						<?php endif; ?>
						<?php if ($canDelete): ?>
    ...
    <?php if ($canCreate): ?>
        <a href="<?php echo JRoute::_('index.php?option=com_hrm&task=positionform.edit&id=0', false, 2); ?>"
...
<script type="text/javascript">
    ...
    function deleteItem() {
        ...
        if (confirm("<?php echo JText::_('COM_HRM_DELETE_MESSAGE'); ?>")) {
            window.location.href = '<?php echo JRoute::_('index.php?option=com_hrm&task=positionform.remove&id=', false, 2) ?>' + item_id;
        }
    }
</script>
