...
<form action="<?php echo JRoute::_('index.php?option=com_hrm&view=historyitselfs'); ?>" method="post" name="adminForm" id="adminForm">

    <?php echo JLayoutHelper::render('default_filter', array('view' => $this), dirname(__FILE__)); ?>
    <table class="table table-striped" id = "historyitselfList" >
        <thead >
            <tr >
    ...
    <?php endif; ?>
    				...
    				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_HRM_HISTORYITSELFS_EMPLOYEE_GUID', 'a.employee_guid', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_HRM_HISTORYITSELFS_OPTION', 'a.option', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_HRM_HISTORYITSELFS_INFORMATION', 'a.information', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>
				<?php echo JHtml::_('grid.sort',  'COM_HRM_HISTORYITSELFS_DATE', 'a.date', $listDirn, $listOrder); ?>
				</th>
    ...
    <?php endif; ?>
					...
					<th class="center">
				<?php echo JText::_('COM_HRM_HISTORYITSELFS_ACTIONS'); ?>
				</th>
				<?php endif; ?>
    ...
    <?php foreach ($this->items as $i => $item) : ?>
        ...
        <tr class="row<?php echo $i % 2; ?>">
            ...
            <?php if (isset($this->items[0]->state)): ?>
                ...
                <td class="center">
                    <a class="btn btn-micro <?php echo $class; ?>"
                       href="<?php echo ($canEdit || $canChange) ? JRoute::_('index.php?option=com_hrm&task=historyitself.publish&id=' . $item->id . '&state=' . (($item->state + 1) % 2), false, 2) : '#'; ?>">
                        <?php if ($item->state == 1): ?>
            ...
            <?php endif; ?>
					...
					<td class="center">
						<?php if ($canEdit): ?>
							<a href="<?php echo JRoute::_('index.php?option=com_hrm&task=historyitselfform.edit&id=' . $item->id, false, 2); ?>" class="btn btn-mini" type="button"><i class="icon-edit" ></i></a>
						<?php endif; ?>
						<?php if ($canDelete): ?>
    ...
    <?php if ($canCreate): ?>
        <a href="<?php echo JRoute::_('index.php?option=com_hrm&task=historyitselfform.edit&id=0', false, 2); ?>"
...
<script type="text/javascript">
    ...
    function deleteItem() {
        ...
        if (confirm("<?php echo JText::_('COM_HRM_DELETE_MESSAGE'); ?>")) {
            window.location.href = '<?php echo JRoute::_('index.php?option=com_hrm&task=historyitselfform.remove&id=', false, 2) ?>' + item_id;
        }
    }
</script>
