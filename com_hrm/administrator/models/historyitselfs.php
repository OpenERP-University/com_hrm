 ...
 */
class HrmModelHistoryitselfs extends JModelList {
    ...
    protected function getListQuery() {
        ...
        $query->from('`#__hrm_history_itself` AS a');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.employee_guid LIKE '.$search.'  OR  a.option LIKE '.$search.'  OR  a.information LIKE '.$search.'  OR  a.date LIKE '.$search.' )');
            }
        }
    ...
    public function getItems() {
		...
		foreach ($items as $oneItem) {
			...
			$oneItem->employee_guid = !empty($textValue) ? implode(', ', $textValue) : $oneItem->employee_guid;
					...
					$oneItem->option = JText::_('COM_HRM_HISTORYITSELFS_OPTION_OPTION_' . strtoupper($oneItem->option));
		}
        return $items;
    }
